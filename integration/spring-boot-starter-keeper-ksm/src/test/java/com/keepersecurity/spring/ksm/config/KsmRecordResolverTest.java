...
class KsmRecordResolverTest {
  ...
  void isUidRecognizesPattern() {
    KsmRecordResolver resolver =
        new KsmRecordResolver(new SecretsManagerOptions(new InMemoryStorage((String) null)));
  ...
  void resolvesFolderAndTitleToUid() {
    ...
    KeeperRecordData data =
        new KeeperRecordData("MyRecord", "login", new ArrayList<>(), null, null);
    KeeperRecord record =
        new KeeperRecord(new byte[0], recordUid, folderUid, null, null, data, 0L, List.of());
